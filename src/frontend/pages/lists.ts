...
async function loadLists(container: HTMLDivElement, errorEl: HTMLParagraphElement) {
  container.innerHTML = '';
  errorEl.textContent = '';
...
form.addEventListener('submit', async (e) => {
  ...
  const nameInput = form.querySelector<HTMLInputElement>('#list-name');
  if (!nameInput) return;
...
container.addEventListener('click', async (e) => {
  const btn = (e.target as HTMLElement).closest<HTMLButtonElement>('.list-card__delete');
  if (!btn) return;
