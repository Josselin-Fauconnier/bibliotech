...
const statusEl     = document.querySelector<HTMLParagraphElement>('#status-message')!;
...
function renderBooks(docs: BookDoc[]) : void {
    for (const book of docs) {
        booksGrid.appendChild(createBookCard(book));
    }
}

function createBookCard(book: BookDoc): HTMLElement {
